...
- **The Selector is a turn, not a template**: families S-A..S-P are recipes (`SELECTOR_FRAGMENTS`: needed atoms + plan sites), realised by turning a turn slot in front of a Filter/Portal/Switch/hop/Fusion input/Prism channel/One-Way/Gate/branch/Fusion output into a Selector slot. D110 originally implemented 12 of the 16 conceptual families; D111 later added S-M. S-I, S-K and S-O still do not exist (no decoy/bait builder).
...
## D112 - V5 Phase S3.1 final desktop verification: split J/K certification windows (uncommitted, no version bump, no APK)

Follow-up verification kept the D111 generator exactly as-is: no new mechanics, no J/K target loosening, no V1-V4 edits, `MAX_COLUMNS=8`, and no APK/S4 work.

- **QA budget root cause**: full single-process 50-level J/K windows exceed the practical QA budget because dev-only verification dominates runtime. In the 90s bounded windows, final shortcut probing was the largest phase (J ~43.7s, K ~35.8s), followed by repeated candidate build/generation (J ~18.2s, K ~26.0s), probe, minimality and check work. Selector analysis itself was not the primary bottleneck.
- **J full-equivalent split window** (`from=2601 count=25 stride=4 hist=1 budget=60000` and `from=2701 count=25 stride=4 hist=1 budget=60000`): 50/50 generated, 4 demoted (8.0%), 0 fallbacks, greedy-solvable accepted 0/50, shortcut-stage rejections 5, minimality-stage rejections 3, weighted avg gen 1950.1 ms, max gen 3522 ms.
- **K full-equivalent split window** (`from=2801 count=25` hit budget after 23 generated before 2893, then `from=2893 count=15`, `from=2953 count=12`, all `stride=4 hist=1 budget=60000`): 50/50 generated, 0 demoted, 0 fallbacks, greedy-solvable accepted 0/50, shortcut-stage rejections 6, minimality-stage rejections 17, weighted avg gen 2551.6 ms, max gen 5383 ms.
- **Targeted late-game checks** (`wide=1`): 2500 SF x2, 2800 SE x2, 2900 SD x2 and 3000 SP x3 were Selector-bearing and clean. Exact 2651 generated without a Selector and was the only demoted exact hard sample (attempt 34); nearest Selector-bearing replacement 2647 (SF x2, downstream depth 7) was clean and not demoted. Manual V5 QA should use 2500, 2647, 2800, 2900, 3000 and should not hide that 2651 is a non-Selector sample.
- **Regression**: `selector_verify.tscn` passed 6/6 with 0 problems. Fusion QA set 1-6 passed a bounded temporary verifier (start unsolved, authored solution solves, unique exact solving state/min-move count). Boundary temp check reconfirmed `MAX_COLUMNS=8`, `MAX_LEVEL=3000`, selector first level 2001, 2000 uses V4, 2001/3000 use V5, and QA next at 3000 clamps to 3000. The canonical V1-V4 fingerprint remains the documented D111 hash `271eb766be20a12446676a947b49a3f1`; a fresh throwaway helper used a different serialization and was not recorded as a replacement. `selector_tutorial_verify.tscn` and a bounded T29-T34 smoke scene were attempted but did not complete in the practical headless QA window, so prior S2 exhaustive tutorial evidence remains the source.
- **Status after D112**: S3.1 is READY FOR MANUAL APPROVAL on desktop numbers. S4/APK remains blocked until the user plays/approves the V5 TEST difficulty/readability and Android generation time is measured.

## D113 - S4 external-test presentation mode, pass 1 (uncommitted, no build/upload)

S4 needs a build that looks like a normal finished game to Android external testers and later TestFlight testers, without deleting the internal QA systems that are still valuable.

- **Build modes are explicit now**: `BuildConfig` has `MODE_INTERNAL_QA`, `MODE_EXTERNAL_TEST`, and `MODE_PRODUCTION`. Current source is set to `MODE_EXTERNAL_TEST`. `QA_TOOLS` is true only for `MODE_INTERNAL_QA`; `IS_PRODUCTION_BUILD` is reserved for a final store-production pass.
- **Hidden in external test**: QA Level Select, V3/Fusion/Selector/V5 test Main Menu entries, gameplay QA next (`+50`, NEXT V3/FUSION/SELECTOR/V5), tutorial QA overlay, generator debug tags (`V4/V5 <band>...`, failure tags), unlock-all campaign/tutorial shortcuts, and the QA HUD stack cap override.
- **Preserved**: the QA levels, generators, verifier tools, code paths and docs remain in the repository. Switching back to `MODE_INTERNAL_QA` exposes them again.
- **Player-facing polish**: Main Menu uses `TUTORIALS` and hides the QA spacer when QA tools are off. Android export metadata was changed from `versionCode=69`, `4.8.3-OFFICE-BRANCH-QA` to `versionCode=70`, `4.8.4` to avoid visible temporary QA branding in platform metadata.
- **Not production monetization**: AdMob still uses Google test IDs and production IDs remain empty in source. This is intentional for external-test review until the user explicitly approves live IDs and store compliance work.
- **Not done in this pass**: no APK/AAB/IPA export, no upload, no commit/push, no macOS/Xcode/TestFlight signing, no generator retuning.

## D114 - Production readiness / PR prep (uncommitted, no build/upload)

Goal: no development-only state leaks into a store build, without touching gameplay.

- **`BuildConfig.BUILD_MODE` committed default is now `MODE_PRODUCTION`** (was `MODE_EXTERNAL_TEST`). `QA_TOOLS` was already `false` outside INTERNAL_QA, so every QA control (QA Level Select, V3/FUSION/SELECTOR/V5 TEST, +50, NEXT *, generator HUD tags, tutorial debug overlay, unlock-all flags) is hidden in production; verified by a rendered-scene driver (main menu = CONTINUE / NEW GAME / TUTORIALS / SETTINGS / QUIT; gameplay label `LEVEL n`) and the inverse under INTERNAL_QA. `tools/ci/set_build_mode.sh` switches mode; `stamp_store_config.sh` refuses to run unless the source is production.
- **AdMob**: `AdConfig.USE_TEST_IDS := not BuildConfig.IS_PRODUCTION_BUILD`. Production ids are no longer a source constant: `AdConfig.production_ids()` reads the gitignored `res://config/ad_ids.local.json` that CI writes from secrets (confirmed packed into a real APK; the stamped App ID appeared in the built manifest). `AdConfig.ads_active()` is false when any production id for the platform is missing, and `AdManager._ready` then never loads the SDK/consent: ads off, hints free. Reason: with no free fallback (D98) an empty unit id would have made Hint permanently unavailable.
- **Play Games Game ID and AdMob App IDs** are injected by `tools/ci/stamp_store_config.sh` (the Android lane fails with a clear message when `PLAY_GAMES_GAME_ID` is unset; AdMob ids are required on `v*` tags / production track). Nothing secret is committed; `.gitignore` gained `.env*`, `*service-account*.json`, `*credentials*.json`, `AuthKey_*.p8`, `/config/ad_ids.local.json`.
- **Unchanged on purpose**: `LevelManager.USE_V3_FOR_PROCEDURAL_QA` / `USE_FUSION_PROGRESSION_FOR_QA` (named "QA" but they select V4 generation for real play - a gameplay decision), product ids (one `NO_FORCED_ADS` id for both stores), all generators, levels, saves, stars, hints.
- **Not resolved (blockers)**: privacy policy URL, live AdMob ids, Play Games id, upload keystore, Apple signing/team, IAP products in both consoles, AD_ID permission decision for child-directed audience (STORE_RELEASE.md 6b).
