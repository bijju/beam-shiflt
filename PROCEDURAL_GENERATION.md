...
## Splitter Selector status (2026-09-25)

The Splitter Selector runtime (D108) and tutorials T29-T34 (D109) exist. **Superseded: Selector Phase S3 (D110) built generator V5 / Levels 2001-3000 - see section 20** (V1-V4 output stays fingerprint-identical; ablation, equivalent-state and triviality rules for the Selector now exist in `ProceduralSelectorCheck`).


## 20. Generator V5 - Levels 2001-3000 with the Splitter Selector (Selector Phase S3, D110)

**Status (2026-09-25): IMPLEMENTED / NOT CERTIFIED, UNCOMMITTED, no APK, no version bump. J/K are demoted too often (next phase S3.1, see `NEXT_AI_PROMPT.md`); manual difficulty review is pending - do not call V5 "approved" or Levels 2001-3000 certified.** V1-V4 are frozen (fingerprint identical).

### 20.1 What it is
`ProceduralLevelGenerator.generate(level, 5)` = `ProceduralProgressionV3.generate(level, 5)`: the V4 pipeline (Fusion included) plus Splitter Selector fragments, an extended difficulty contract, Selector-aware complexity/triviality/shortcut analysis and a few V5-only gates. `LevelManager.procedural_generator_version_for_new_play(level)` maps level >= 2001 to V5 (unconditionally - the QA flags only choose between V3/V4 below it); a resumed puzzle always uses its saved version. `INITIAL_CERTIFIED_LEVEL_TARGET = MAX_LEVEL = 3000` (the CURRENT boundary, not a permanent architectural ceiling); `PROCEDURAL_QA_JUMP_AMOUNT` (50) clamps at it (1951 -> 2001, 2951 -> 3000, 3000 no-op).

### 20.2 Contract bands G-K (`ProceduralDifficultyContract._BANDS`, targets - see 20.7 for what is realised)
| Band | Levels | Moves | Depth | Deps | Selector share (realised target) | Selector count | Board |
|---|---|---|---|---|---|---|---|
| G "Selector Entry" (`SEL`) | 2001-2200 | 20-24 | 10-12 | 6-9 | 35-45% | 1 | 8x11 |
| H "Selector Branching" (`SBR`) | 2201-2400 | 22-26 | 11-14 | 7-10 | 45-55% | 1-2 | 8x11 |
| I "Selector Interlock" (`SIL`) | 2401-2600 | 24-28 | 12-15 | 8-11 | 55-65% | 1-2 (3) | 8x11 |
| J "Selector Consequence" (`SCQ`) | 2601-2800 | 26-30 | 13-17 | 9-13 | 60-70% | ~2 | 8x11 |
| K "Selector Mastery" (`SMS`) | 2801-3000 | 28-34 | 15-19 | 10-15 | 65-75% | 2-3 | 8x11 |
Interaction/kind floors (8/5, 10/5, 10/6, 12/6, 15/6) are this pass's own. `_V3_POLICY` gets one row (2001-3000, same discipline as the top V3 row), `_BOARDS` one row (8x10, 8x11; **8x12 is not comfortable - 94 px < 96 px, six of six rejections in the first run**; `_board_for` then picks only 8x11), `_FUSION_PROGRESSION` one row (Fusion keeps its 0.28 roll). `SELECTOR_POLICY` (`selector_policy(level)`) holds roll frequency (0.40/0.50/0.60/0.65/0.76 - calibrated so the REALISED share lands in the target), count weights, the family list per band and `min_downstream_depth` (1/1/2/2/3). Whether a level carries a Selector is `frac(level * 0.618034 + 0.137) < frequency` (low-discrepancy, deterministic, any window of levels sits inside the target - no RNG); Level 2001 always carries one simple Selector (`SELECTOR_INTRO_LEVEL`, entry families SA/SB/SH or SE, one tap). T29 (unlock at procedural Level 1900) is available well before.

### 20.3 Pipeline additions (all gated so V1-V4 are byte-identical)
1. **Rolls** (dedicated rng streams, once per level): Fusion roll as in V4 (stream 90); Selector roll `ProceduralFragmentsV3.roll_selector` (stream 91): family (Fusion families SE/SF/SP are down-weighted 0.5 and forced onto a Fusion recipe if the level did not roll one; if Fusion rolled, only Fusion families are eligible), count.
2. **Plan**: `choose_atoms` gets the family's `need` (any-of atom groups; a missing one is appended) and counts each Selector as one node/dependency/kind in the predicted depth. V5 cores/escalation are weighted by nodes per tile and by measured layout reliability (`V5_LAYOUT_WEIGHT`: SO/SH/PG fail 75-90% of layouts); TM/OH patterns are rarer. `_apply_selectors` turns family sites (a turn slot in front of a Filter/Portal/Switch/Receiver hop/Fusion input/Prism channel/One-Way/Gate/Splitter branch/Fusion output) into Selector slots; extras prefer the line of an earlier Selector (series decisions). Tap distance per Selector 1/2/3 with 55/35/10% (Level 2001: 1); the extra taps join the fixed moves.
3. **Density fit** (`compose`): `tiles = BASE + turns + special-tile costs` against `v5_tile_budget` (56); if the atoms leave room for fewer moves than 75% of the band floor the plan is redrawn (cheap), otherwise the move target shrinks to what fits. After more than 3 layout failures each further one shaves 1 move (at most 25% of the floor). Move draw is spread over the band (`r^1.4`).
4. **Layout** (`ProceduralComposerV3`): kind `"selector"` is a turn (`Cursor.to_selector_turn`, physical directions like Fusion); `_bias_selector` prefers cells whose two wrong rays already run into a placed tile; per-plan search budgets (`restarts`/`op_budget`/`walk_budget`).
5. **Hardening**: a Selector's three wrong rays are SCREENED with the real LaserSystem (`harden` -> `_selector_state_solves`) - only a wrong state that solves gets a blocker; one that still solves is unrepairable (rejects).
6. **Gates** (`ProceduralSelectorCheck`, see 20.4) then `_extra_gates`, then the pre-probe (default 420/12), greedy, **`ProceduralMinimality`**, then ONE final wide probe (`V5_FINAL_PROBE_SIMS` 3200, width 48) on the survivor.
7. **Attempts**: 40 (V3/V4: 12-16). `SELECTOR_ATTEMPTS` 30 keep the rolled family; a level that rolled a Selector demotes its reasoning floors from attempt 8 (`V5_DEMOTE_AFTER_SELECTOR`), others from attempt 14 (`V5_DEMOTE_AFTER`) to the floors of the band BELOW (moves, dependencies, depth, interactions, kinds) - the Selector rules are never demoted except the strict-band extras. Only then V2 fallback (`V5_GENERATION_FAILED`).

### 20.4 Selector analysis (`scripts/procedural/procedural_selector_check.gd`, runtime-safe, LaserSystem only)
Per Selector, on the solved board (5 simulations): **blocker ablation** (must lose a target = load-bearing), **empty-cell removal** (must not stay solved), the **three other orientations** (none may solve: no shortcut/equivalent state), **signatures** (`_full_sig` over lit targets/switches/receivers/gates/fusion colours + touched cells: `selector_equivalent_state_count`, `distinct_wrong`), **consequence** (a wrong state is consequential when it differs from the dead state or newly touches a non-mirror mechanic), **live outputs** (< 2 => the correct output is obvious), **live wrong rays**, **downstream depth** (load-bearing units on the beam after the Selector, fixed-point over splitter/prism/fusion/remote children). `mirror_like` = no consequential wrong state and < 2 live wrong rays. `counted` = load-bearing, not mirror-like, no solving wrong state; only counted Selectors add the `selector` kind/dependency/depth in `ProceduralComplexity.analyze`. `reasons_for(sel, min_downstream, strict)` adds: several Selectors must be **coupled** (a wrong state of one changes what reaches another) or **converging** (shared downstream cells or the same lost target) - otherwise "independent one-step decisions"; the deepest decision must feed >= `min_downstream_depth` mechanics; strict bands (>= 2601, not demoted) also need one consequential Selector and one with >= 2 distinct wrong outcomes. Metrics exposed in `metrics`: `selector_count`, `selector_load_bearing_count`, `selector_dependency_count`, `selector_interaction_count`, `selector_equivalent_state_count`, `selector_downstream_depth` (+ `metrics["selector"]` with per-Selector detail). `ProceduralComplexity.analyze` counts 4-state tap distance (`intended_move_count`), records `padding_positions`, ablates Selectors as a dead BLOCKER (D101 lesson).

### 20.5 Family catalog (`ProceduralFragmentsV3.SELECTOR_FRAGMENTS`; ids are stored in `selector_fragment`)
SA Selector->Filter, SB ->Portal, SC ->Switch->Gate, SD ->Receiver->Remote, SE ->Fusion input, SF two Selectors->two Fusion inputs, SG Prism->Selector->colour route, SH ->One-Way, SJ ->Portal->Receiver->Remote, SL Selector in front of a Gate, SM Selector->target continuation, SN Selector on a Splitter branch, SP Fusion output + input Selectors. **Not implemented**: S-I shared-mirror Selector, S-K target-vs-prerequisite bait, S-O indirect two-Selector dependency (the composer has no bait/decoy builder; the consequence metric and the wrong-ray screen approximate their intent). Realisation per family is uneven: in Consequence/Mastery windows SB/SD/SE/SF/SG/SJ/SN/SP realise 75-100% of their rolls, SC/SL/SM can still drop when no suitable site survives layout and strict Selector checks.

### 20.6 Minimality and shortcuts (why they exist)
Replaying an intended solution through a real GridManager showed the puzzle SOLVED BEFORE ALL intended taps in ~14% of unscreened Mastery boards (e.g. 10 of 26 tiles sufficed): a whole group of tiles was superfluous although each tile alone was "required" (so the per-tile padding check and the shallow probe missed it). `ProceduralMinimality.find_cheaper` (runtime, LaserSystem only): lazy-activation replay, ddmin over ~11 tile orders (activation stage + composed line, positions, seeded shuffles) and unions of 1-3 whole lines, budget 900 simulations; a hit rejects the attempt (`minimality` stage, 4-11 per 50 levels). A wide final probe found further 9-14-flip alternatives (e.g. 22 intended, 9 needed) in ~2% of Entry/Branching boards. **Residual** (independent screens, sample sizes small): random-order ddmin x4 still found a superfluous group in 1 of 157 sampled levels (Entry/Consequence/Mastery windows, ~0.6%, measured before the last ordering additions); a 6000-simulation/80-width probe found a shortcut in 3 of 72 Interlock/Consequence/Mastery levels (~4%, 12 flips vs 24+) and in 0 of 50 Entry/Branching levels that the runtime screens had accepted. These are screens, not proofs; the exact search (20.8) can prove nothing at 20+ moves.

### 20.7 Measured evidence (desktop headless, 50-level windows, stride 4, five parallel processes; timings therefore slightly inflated)
| Band | Selector realised | avg count | moves (avg) | depth (avg) | deps (avg) | demoted | Selector dropped | fallbacks | gen ms avg (max) |
|---|---|---|---|---|---|---|---|---|---|
| G 2001-2200 | 42% (21/50) | 1.00 | 19-24 (21.2) | 10-13 (11.0) | 6-10 (7.8) | 0 | 0 | 0 | 1501 (4916) |
| H 2201-2400 | 50% (25/50) | 1.40 | 20-25 (22.8) | 10-14 (11.9) | 7-10 (8.3) | 5 | 0 | 0 | 1682 (2902) |
| I 2401-2600 | 58% (29/50) | 1.86 | 20-28 (24.0) | 11-16 (12.9) | 7-12 (9.4) | 9 | 1 | 0 | 1848 (3408) |
| J 2601-2800 | 62% (31/50) | 2.03 | 19-30 (24.8) | 12-16 (13.7) | 8-12 (9.7) | 26 | 3 | 0 | 1979 (3581) |
| K 2801-3000 | 71% (34/48) | 2.24 | 21-31 (25.6) | 13-17 (14.6) | 9-13 (10.4) | 31 | 4 | 0 | 2306 (5209) |
**Honest reading:** frequency targets are met in all five bands. Moves reach the band floor in G/H, sit at its bottom in I and BELOW it in J/K (K averages 25.6 of 28-34): 28-34 moves with depth 15+ need ~65+ tiles on an 8x11 board (>75% occupancy), which the composer cannot place (60-90% layout failure) and which would be clutter. About half of J and K levels are `band_demoted` (they meet the reasoning floors of the band below - depth 13 instead of 15 in K). Greedy beam-following solved 0 of 250 accepted levels (a heuristic, not a human measure). Timing includes the final probe (~1 s/level) and minimality (~0.25 s/level); generation on a real phone is NOT measured and is likely 3-5x slower - S4 must measure it (caching or prefetching the result is the obvious mitigation).

### 20.8 Verification (dev-only tools under `scripts/tools/`, never exported)
- `v5_sample.tscn`: anchors / windows / `family=XX` / `nosel=1` / `hist=1` (rejection histogram, per-atom layout failure, rolled-vs-realised family) / `wide=1|big` (independent 2500/40 or 6000/80 probe) / `ascii=1` / `budget=`. Prints `QA_BUDGET_EXCEEDED` instead of running on.
- `v5_verify.tscn`: determinism (two generations identical), start unsolved / intended solves, **independent Selector brute force** (exactly ONE of 4 orientations solves; dead blocker loses the level) and an **exact touched-tile BFS** (exact for the shortest solution because in any unsolved state at least one differing tile is touched); it reports PROVEN only when the search exhausted every state cheaper than intended - on V3 levels 5-150 it proves 8/8, on V5 anchors it is UNKNOWN (state budget exhausted at depth 9-19 of 20+). **`verified_optimal_moves` is -1 for every V5 level** and `StarScoring` falls back to `intended_moves`.
- Real-game drivers (temporary, deleted): full Save/Continue at 2001/2200/2500/2800/3000 (level, generator version 5, seed, every orientation incl. Selector states, move count, hint-used all restored exactly, regeneration identical); Hint (wrong Selector is the hint target, a hint never rotates or counts a move, hint-used caps stars at 2); replay of the intended solution through a real GridManager for 7 anchors (solved, taps == `intended_moves`, 3 stars); QA +50 (1951->2001, 2001->2051, 2951->3000, 3000->3000, 2960->3000, 2000->2050; no stars, no real-progression change); 2000 -> 2001 through the real Next-Level path (V4 -> V5); rendered V5 TEST screenshots.
- Determinism across processes: 22 anchor levels hashed twice in separate runs - identical. V1-V4 fingerprint (33 levels x 4 versions = 132) identical to the pre-S3 baseline.
